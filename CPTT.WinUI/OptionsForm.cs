							...
							{         
								...
								for ( int tables=0; tables<classNumbers; tables++ )
								{
									...
									foreach ( DataRow infoRow in dsStuInfoXls.Tables[tables].Rows )
									{
                                        ...
                                        option.StuID = infoRow[24] == null ? guid.ToString() : infoRow[24].ToString();
									...
									ToTypeInfo:
										...
										for ( int cardSeq=1,ver=0; cardSeq<=5; cardSeq++,ver++ )
										{
											...
											ToStuCardInfo:
												int cardAffected = optionSystem.InsertStuCardInfo(option.StuID,infoRow[cardSeq+12+ver].ToString().Replace(" ",""),infoRow[cardSeq+13+ver].ToString().Replace(" ",""),Convert.ToDateTime(infoRow[23]),cardSeq);
												if ( cardAffected == -1 )
												{
