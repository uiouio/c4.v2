			...
			string getBegDate,string getEndDate,string savePath)
		{
			try
			{
					...
					if ( !getOutputName.Equals("") || !getOutputNumber.Equals(""))
					{
						if ( dsHealthOutput.Tables[0].Rows.Count > 0 )
						{
							...
							for ( int i=0; i<dsHealthOutput.Tables[0].Rows.Count; i++ )
							{
								...
								{
                                    m_objRange = m_objSheet.get_Range((Excel.Range)m_objSheet.Cells[i + 4, j - 1], (Excel.Range)m_objSheet.Cells[i + 4, j - 1]);
							...
							if ( getOutputClass.Equals("") )
							{
								...
								for ( int i=0; i<dsHealthOutput.Tables[0].Rows.Count; i++ )
								{
									...
									{ 
										...
										for ( int j=2; j<dsHealthOutput.Tables[0].Columns.Count-1; j++ )
										{
                                            m_objRange = m_objSheet.get_Range((Excel.Range)m_objSheet.Cells[rowBeg, j - 1], (Excel.Range)m_objSheet.Cells[rowBeg, j - 1]);
											m_objRange.NumberFormatLocal = "@";
											if ( j != 5 )
												m_objRange.set_Item(1,1,dsHealthOutput.Tables[0].Rows[i][j].ToString());
											else
				...
				if ( printType2nd )
				{
					...
					{
                        m_objRange = m_objSheet.get_Range((Excel.Range)m_objSheet.Cells[i + 6, 1], (Excel.Range)m_objSheet.Cells[i + 6, 1]);
							...
							for ( int i=0; i<dsHealthOutput.Tables[0].Rows.Count; i++ )
							{
								for ( int j=2; j<dsHealthOutput.Tables[0].Columns.Count; j++ )
								{
									...
									else
									{
										if (j == 14)
										{
                                            m_objRange = m_objSheet.get_Range((Excel.Range)m_objSheet.Cells[i + 4, 12], (Excel.Range)m_objSheet.Cells[i + 4, 12]);
											m_objRange.NumberFormatLocal = "@";
											m_objRange.set_Item(1,1,dsHealthOutput.Tables[0].Rows[i][j].ToString());
										}
										if (j == 17)
										{
                                            m_objRange = m_objSheet.get_Range((Excel.Range)m_objSheet.Cells[i + 4, 13], (Excel.Range)m_objSheet.Cells[i + 4, 13]);
											m_objRange.NumberFormatLocal = "@";
											m_objRange.set_Item(1,1,dsHealthOutput.Tables[0].Rows[i][j].ToString());
										}
									...
									{ 
										...
										for ( int j=2; j<dsHealthOutput.Tables[0].Columns.Count; j++ )
										{
											...
											else
											{
												if (j == 14)
												{
                                                    m_objRange = m_objSheet.get_Range((Excel.Range)m_objSheet.Cells[rowBeg, 12], (Excel.Range)m_objSheet.Cells[rowBeg, 12]);
												...
												if (j == 17)
												{
                                                    m_objRange = m_objSheet.get_Range((Excel.Range)m_objSheet.Cells[rowBeg, 13], (Excel.Range)m_objSheet.Cells[rowBeg, 13]);
...
//						objData[i,11] = dv[i][14].ToString();
				...
				if ( printType2nd )
				{
					...
					for ( int i=0; i<dsClass.Tables[0].Rows.Count; i++ )
					{
                        m_objRange = m_objSheet.get_Range((Excel.Range)m_objSheet.Cells[i + 7, 1], (Excel.Range)m_objSheet.Cells[i + 7, 1]);
						m_objRange.Value = dsClass.Tables[0].Rows[i][1].ToString();
					...
					for ( int row=0; row<dsClass.Tables[0].Rows.Count; row++ )
					{
						...
						{
                            m_objRange = m_objSheet.get_Range((Excel.Range)m_objSheet.Cells[6, column], (Excel.Range)m_objSheet.Cells[6, column]);
							...
							if ( column < 10 )
							{
								...
								if ( dsHeightAnalyStat.Tables[0].Rows.Count != 0 )
								{
                                    ...
                                    m_objRange = m_objSheet.get_Range((Excel.Range)m_objSheet.Cells[row + 7, column], (Excel.Range)m_objSheet.Cells[row + 7, column]);
								...
								if ( row+1 == dsClass.Tables[0].Rows.Count )
								{
									if (column < 9)
									{
										...
										if ( dsHeightTotal.Tables[0].Rows.Count != 0 )
										{
                                            ...
                                            m_objRange = m_objSheet.get_Range((Excel.Range)m_objSheet.Cells[dsClass.Tables[0].Rows.Count + 7, column],
                                                (Excel.Range)m_objSheet.Cells[dsClass.Tables[0].Rows.Count + 7, column]);
