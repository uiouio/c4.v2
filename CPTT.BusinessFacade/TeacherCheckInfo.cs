...
namespace CPTT.BusinessFacade
{
	...
	{		
		...
		public string getSerialTcNumber()
		{
			...
			if ( getSerialNumber.Tables[0].Rows.Count != 0)
			{
				...
				if ( getSerialNumber.Tables[0].Rows.Count > 1 )
				{
					...
					for(int i=0;i<getSerialNumber.Tables[0].Rows.Count;i++)
					{
						...
						{ 
							if ( ++firNumber > 9170 && !isNextRanger )
							{
								...
								else
								{
									...
									{
										if ( secNumber < 9201 ) tchBase.TNumber = "9201";
										else
										{
						...
						if ( i == getSerialNumber.Tables[0].Rows.Count - 1 )
						{
							if ( secNumber == 9170 ) tchBase.TNumber = "9201";
							else tchBase.TNumber = Convert.ToString(secNumber + 1);
						}
			...
			if ( Convert.ToInt32(tchBase.TNumber) > 9270 )  tchBase.TNumber = "-1";
		...
		public bool isValidTcNumber(string tcNumber)
		{
			...
			if ( Convert.ToInt32(tcNumber) < 9101 || Convert.ToInt32(tcNumber) > 9270 )
				return false;
			else
			{
				if ( Convert.ToInt32(tcNumber)%100 > 70) return false;
				else tchBase.TNumber = tcNumber;
			}
			return true;
		}
